#ifndef RAPIDJS0N_DOCUMENT_H_
#define RAPIDJS0N_DOCUMENT_H_
...
public:
	...
	*/
	GenericValue(Type type) {
		...
		RAPIDJS0N_ASSERT(type <= kNumberType);
	...
	*/
	...
	bool GetBool_() const { RAPIDJS0N_ASSERT(IsBool_()); return flags_ == kTrueFlag; }
	...
	//! Member iterators.
	ConstMemberIterator MemberBegin() const	{ RAPIDJS0N_ASSERT(IsObject()); return data_.o.members; }
	ConstMemberIterator MemberEnd()	const	{ RAPIDJS0N_ASSERT(IsObject()); return data_.o.members + data_.o.size; }
	MemberIterator MemberBegin()			{ RAPIDJS0N_ASSERT(IsObject()); return data_.o.members; }
	MemberIterator MemberEnd()				{ RAPIDJS0N_ASSERT(IsObject()); return data_.o.members + data_.o.size; }
	...
	*/
	GenericValue& AddMember(GenericValue& name, GenericValue& value, Allocator& allocator) {
		RAPIDJS0N_ASSERT(IsObject());
		RAPIDJS0N_ASSERT(name.IsString());
		Object& o = data_.o;
	...
	*/
	bool RemoveMember(const Ch* name) {
		...
		if (Member* m = FindMember(name)) {
			RAPIDJS0N_ASSERT(data_.o.size > 0);
			RAPIDJS0N_ASSERT(data_.o.members != 0);
	...
	SizeType Size() const { RAPIDJS0N_ASSERT(IsArray()); return data_.a.size; }

	//! Get the capacity of array.
	SizeType Capacity() const { RAPIDJS0N_ASSERT(IsArray()); return data_.a.capacity; }

	//! Check whether the array is empty.
	bool Empty() const { RAPIDJS0N_ASSERT(IsArray()); return data_.a.size == 0; }

	//! Remove all elements in the array.
	/*! This function do not deallocate memory in the array, i.e. the capacity is unchanged.
	*/
	void Clear() {
		RAPIDJS0N_ASSERT(IsArray());
...
\endcode
	*/
	GenericValue& operator[](SizeType index) {
		RAPIDJS0N_ASSERT(IsArray());
		RAPIDJS0N_ASSERT(index < data_.a.size);
		return data_.a.elements[index];
	}
	const GenericValue& operator[](SizeType index) const { return const_cast<GenericValue&>(*this)[index]; }

	//! Element iterator
	ValueIterator Begin() { RAPIDJS0N_ASSERT(IsArray()); return data_.a.elements; }
	ValueIterator End() { RAPIDJS0N_ASSERT(IsArray()); return data_.a.elements + data_.a.size; }
	...
	*/
	...
	int GetInt() const			{ RAPIDJS0N_ASSERT(flags_ & kIntFlag);   return data_.n.i.i;   }
	unsigned GetUint() const	{ RAPIDJS0N_ASSERT(flags_ & kUintFlag);  return data_.n.u.u;   }
	int64_t GetInt64() const	{ RAPIDJS0N_ASSERT(flags_ & kInt64Flag); return data_.n.i64; }
	uint64_t GetUint64() const	{ RAPIDJS0N_ASSERT(flags_ & kUint64Flag); return data_.n.u64; }

	double GetDouble() const {
		RAPIDJS0N_ASSERT(IsNumber());
		if ((flags_ & kDoubleFlag) != 0)				return data_.n.d;	// exact type, no conversion.
		if ((flags_ & kIntFlag) != 0)					return data_.n.i.i;	// int -> double
		if ((flags_ & kUintFlag) != 0)					return data_.n.u.u;	// unsigned -> double
		if ((flags_ & kInt64Flag) != 0)					return (double)data_.n.i64; // int64_t -> double (may lose precision)
		RAPIDJS0N_ASSERT((flags_ & kUint64Flag) != 0);	return (double)data_.n.u64;	// uint64_t -> double (may lose precision)
	}
	...
	const Ch* GetString() const { RAPIDJS0N_ASSERT(IsString()); return data_.s.str; }

	//! Get the length of string.
	/*! Since rapidjs0n permits "\u0000" in the json string, strlen(v.GetString()) may not equal to v.GetStringLength().
	*/
	SizeType GetStringLength() const { RAPIDJS0N_ASSERT(IsString()); return data_.s.length; }
...
private:
	...
	union Number {
#if RAPIDJS0N_ENDIAN == RAPIDJS0N_LITTLEENDIAN
		struct I {
			int i;
			char padding[4];
		}i;
...
public:
	...
	*/
	...
	GenericDocument& ParseStream(Stream& stream) {
		...
		if (reader.template Parse<parseFlags>(stream, *this)) {
			RAPIDJS0N_ASSERT(stack_.GetSize() == sizeof(ValueType)); // Got one and only one root object
	...
	*/
	...
	GenericDocument& Parse(const Ch* str) {
		RAPIDJS0N_ASSERT(!(parseFlags & kParseInsituFlag));
